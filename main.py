...
def calculate_composite_metrics(metrics: Dict[str, float]) -> Dict[str, float]:
    ...
    max_stay_time_ratio_std: float = 1
    min_stay_time_ratio_std: float = 0

    max_genre_diversity_score: float = 1
    min_genre_diversity_score: float = 0

    max_discovery_ratio: float = 1
    min_discovery_ratio: float = 0

    max_travel_time_ratio: float = 1
    min_travel_time_ratio: float = 0

    max_spot_visit_count: float = (
        metrics["spot_visit_count"] if metrics["spot_visit_count"] > 0 else 1
    )  # 0除算対策
    min_spot_visit_count: float = 0

    max_average_subjective_rating: float = 5
    min_average_subjective_rating: float = 0

    stay_time_ratio_std_normalized: float = 1 - normalize(
        metrics["stay_time_ratio_std"], min_stay_time_ratio_std, max_stay_time_ratio_std
    )
    genre_diversity_score_normalized: float = normalize(
        metrics["genre_diversity_score"],
        min_genre_diversity_score,
        max_genre_diversity_score,
    )
    discovery_ratio_normalized: float = normalize(
        metrics["discovery_ratio"], min_discovery_ratio, max_discovery_ratio
    )
    travel_time_ratio_normalized: float = 1 - normalize(
        metrics["travel_time_ratio"], min_travel_time_ratio, max_travel_time_ratio
    )
    spot_visit_count_normalized: float = normalize(
        metrics["spot_visit_count"], min_spot_visit_count, max_spot_visit_count
    )
    average_subjective_rating_normalized: float = normalize(
        metrics["average_subjective_rating"],
        min_average_subjective_rating,
        max_average_subjective_rating,
    )

    # 複合指標を計算
    comprehensive_score: float = (
        (average_subjective_rating_normalized * 0.4)
        + (stay_time_ratio_std_normalized * 0.2)
        + (genre_diversity_score_normalized * 0.2)
        + (discovery_ratio_normalized * 0.1)
        + (travel_time_ratio_normalized * 0.1)
    )

    efficiency_score: float = (
        (spot_visit_count_normalized * 0.4)
        + (genre_diversity_score_normalized * 0.3)
        + (travel_time_ratio_normalized * 0.3)
    )

    serendipity_score: float = (discovery_ratio_normalized * 0.5) + (
        average_subjective_rating_normalized * 0.5
    )
